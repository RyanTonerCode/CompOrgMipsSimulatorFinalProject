...
bool dataHazard(string currentLine, string prevLine) {
	...
	string dest_str = prevLine.substr(prevLine.find("$"), prevLine.find(","));
	...
	if (instruction == "add" || instruction == "or" || instruction == "and" || instruction == "slt") {
		...
		string reg1 = currentLine.substr(currentLine.find(",")).substr(currentLine.find("$"), currentLine.find(","));
		...
		string reg1 = currentLine.substr(currentLine.find("$"), currentLine.find(","));
		string reg2 = currentLine.substr(currentLine.find_last_of("$"), currentLine.find_last_of(","));
